...
// 🚀 OPTIMIZED: Lazy load with better loading states
const Navbar = dynamic(() => import('@/components/Navbar'), { 
  ssr: false,
  loading: () => <div style={{ height: '96px', background: 'transparent' }} />
});

const NewsletterForm = dynamic(() => import('@/components/NewsletterForm'), { 
  ssr: false,
  loading: () => <div style={{ height: '200px', background: 'transparent' }} />
});

// 🎯 OPTIMIZED: Memoized SVG Icons - ป้องกัน re-render
...
// 🚀 OPTIMIZED: Memoized Social Links - ป้องกัน re-render
const SocialLinks = memo(() => {
  const socialData = useMemo(() => [
    { href: "https://www.facebook.com/UndaAlunda", icon: FacebookIcon, label: "Follow Unda Alunda on Facebook" },
    { href: "https://www.youtube.com/@undaalunda", icon: YoutubeIcon, label: "Watch Unda Alunda on YouTube" },
    { href: "https://www.instagram.com/undalunda", icon: InstagramIcon, label: "Follow Unda Alunda on Instagram" },
    { href: "https://open.spotify.com/artist/021SFwZ1HOSaXz2c5zHFZ0?si=JsdyQRqGRCGYfxU_nB_qvQ", icon: SpotifyIcon, label: "Listen to Unda Alunda on Spotify" },
    { href: "https://twitter.com/undaalunda", icon: TwitterIcon, label: "Follow Unda Alunda on Twitter/X" },
    { href: "https://www.threads.net/@undalunda", icon: ThreadsIcon, label: "Follow Unda Alunda on Threads" }
  ], []);
...
// 🚀 OPTIMIZED: Memoized Footer Links - ป้องกัน re-render
const FooterLinks = memo(() => {
  const footerData = useMemo(() => [
    { href: "/shipping-and-returns", text: "SHIPPING & RETURNS" },
...
export default function AppClientWrapper({ children }: { children: React.ReactNode }) {
  ...
  const layoutClassName = useMemo(() => 
    `min-h-screen w-full relative transition-opacity duration-500 ${
  ...
  useEffect(() => {
    ...
    // 🎯 OPTIMIZED: Browser native scroll restoration
  ...
  }, [pathname, setLastActionItem]);

  return (
    <>
      <Navbar />

      <div id="__layout" className={layoutClassName}>
        {children}
        <CartSuccessPopup />
