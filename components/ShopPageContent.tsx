...
export default function ShopPageContent({ tabParam }: Props) {
  ...
  return (
    <main className="shop-page-main min-h-screen text-[#f8fcdc] font-[Cinzel] px-4 pt-50 pb-4">
      ...
      {activeTab === 'DIGITAL' ? (
        <p className="text-center w-full text-[#dc9e63] font-[Cinzel] mt-10 text-lg opacity-60">
          เดี๋ยวมาทำต่อ นอนแป๊บ
        </p>
      ) : (
        <div className="stems-row">
          {itemsToRender.map((item, i) => (
            <Link href="/shop" key={i} className="stems-item product-label-link">
              <img src={item.src} alt={item.title} className="stems-image" width={200} height={200} />
